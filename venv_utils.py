...
def setup_venv_with_pip(venv_dir, force_recreate=False):
    ...
    if not pip_available:
        ...
        pip_install_methods = [
            ...
            ([sys.executable, "-m", "pip", "install", "--target", str(venv_path / "lib"), "pip"], "system pip to venv"),
            
            # Method 4: Copy system pip to venv
            ([sys.executable, "-m", "pip", "install", "--force-reinstall", "--target", str(venv_path), "pip"], "force install pip to venv"),
        ]
        ...
        for cmd, method_name in pip_install_methods:
            ...
            if success:
                ...
                if success:
                    ...
                    print(f"⚠️ pip installation succeeded but still not working: {stderr}")
            else:
                print(f"⚠️ {method_name} failed: {stderr}")
        else:
    ...
    success, stdout, stderr = run_command([str(python_exe), "-m", "pip", "install", "--upgrade", "pip"])
    if not success:
...
def download_packages_for_offline_use(packages, download_dir="offline_packages"):
    ...
    print(f"📥 Downloading packages for offline use to: {download_path.absolute()}")
    ...
    if not success:
        print(f"⚠️ Initial download failed, trying with dependencies: {stderr}")
    ...
    downloaded_files = list(download_path.glob("*.whl")) + list(download_path.glob("*.tar.gz"))
...
def install_packages_offline(python_exe, packages, offline_dir):
    ...
    for package in packages:
        ...
        if not success:
            ...
            matching_files = [f for f in package_files if package_name.lower() in f.name.lower()]
            
            if matching_files:
                print(f"   Trying to install specific files for {package_name}...")
...
def install_packages_online(python_exe, packages):
    ...
    for package in packages:
        ...
        success, stdout, stderr = run_command([str(python_exe), "-m", "pip", "install", package])
        ...
        if not success:
            ...
            fallback_methods = [
                ...
                ([str(python_exe), "-m", "pip", "install", "--user", package], "--user flag"),
