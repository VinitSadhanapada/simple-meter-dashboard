...
# View to show latest 10 meter readings
...
FAILURE_MODES_FILE = '/home/pi/Desktop/dash/simple-meter-dashboard/failure_modes.json'
...
def save_failure_modes(modes):
    try:
        print(f"[DEBUG] Writing to FAILURE_MODES_FILE: {os.path.abspath(FAILURE_MODES_FILE)}")
        with open(FAILURE_MODES_FILE, 'w') as f:
            json.dump(modes, f, indent=2)
        print(f"[DEBUG] Saved failure modes: {modes}")
    except Exception as e:
        print(f"[ERROR] Could not write to {FAILURE_MODES_FILE}: {e}")
        raise

def get_meter_list():
    # Load from device_config.jsonc
    config_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'device_config.jsonc')
    try:
...
def api_set_failure_mode(request):
    ...
    try:
        ...
        print(f"[DEBUG] api_set_failure_mode: Set {meter_name} to {mode}")
        return JsonResponse({'status': 'ok', 'meter_name': meter_name, 'mode': mode})
    except Exception as e:
        print(f"[ERROR] api_set_failure_mode: {e}")
