-- tools/clear_meter_readings_keep_drishti.sql
--
-- Safe script to backup and remove all meter readings EXCEPT those
-- belonging to the DrishtiStudio Raspberry Pi. This file uses the
-- `meter_readings` table name (with underscore) as requested.
--
...
-- pg_dump -U <DB_USER> -d <DB_NAME> -t public.meter_readings -Fc -f meter_readings_backup.dump

-- 2) Optional quick SQL backup (creates a copy table with timestamp):
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'meter_readings_backup') THEN
    CREATE TABLE public.meter_readings_backup AS TABLE public.meter_readings WITH NO DATA;
  END IF;
END$$;

INSERT INTO public.meter_readings_backup
