...
def require_sudo_for_command(command_name):
    ...
    if os.geteuid() != 0 and not check_sudo_available():
        print(f"❌ Error: '{command_name}' requires sudo access for systemd operations.")
        print("")
        print("💡 Solutions:")
...
class SimpleDashboard:
    ...
    def check_prerequisites(self):
        ...
        for package in required_packages:
            success, stdout, stderr = self.run_command(['dpkg', '-l', package], check=False)
        ...
        # Check venv module
        success, stdout, stderr = self.run_command([sys.executable, '-m', 'venv', '--help'], check=False)
        if success:
        ...
        success, stdout, stderr = self.run_command([sys.executable, '-m', 'pip', '--version'], check=False)
        if success:
...
"""
    ...
    def install_crontab(self):
        ...
        # Get current crontab
        success, current_cron, _ = self.run_command(["crontab", "-l"], check=False)
        ...
        if not success or cron_entry not in current_cron:
            ...
            success, stdout, stderr = self.run_command(["crontab", "/tmp/new_crontab"])
                ...
                if os.path.exists(CONFIG["PORT"]):
                    try:
                        ...
                        if client.connect():
                            ...
                            self.logger.warning("Failed to connect, using simulation mode")
                            CONFIG["SIMULATION_MODE"] = True
                    except Exception as e:
                        self.logger.error(f"Hardware error: {e}, using simulation mode")
                        CONFIG["SIMULATION_MODE"] = True
                else:
                    self.logger.warning(f"Port {CONFIG['PORT']} not found, using simulation mode")
                    CONFIG["SIMULATION_MODE"] = True
...
def main():
    ...
    parser.add_argument("--create-service", action="store_true", help="Create systemd service file (requires sudo)")
    parser.add_argument("--install", action="store_true", help="Full install: setup + create service (requires sudo)")
    parser.add_argument("--run", action="store_true", help="Run dashboard")
    parser.add_argument("--run-service", action="store_true", help="Run dashboard as service (used by systemd)")
