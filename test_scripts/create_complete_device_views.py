#!/usr/bin/env python3
"""
Create complete device_config views that work with the copied DCMS templates
"""


def create_complete_views():
    """Create complete views.py file for device_config app"""
...
class DeviceConfigView(View):
    """Main device configuration management view - matches device_list.html template"""
    ...
    def get(self, request):
        ...
        # Convert to format expected by DCMS templates
        devices = []
        for i, config in enumerate(pi_configs):
            device = {
                ...
                'last_connected': 'Just now',  # Placeholder
...
class AddPiView(View):
    """Add new Pi configuration - matches add_device.html template"""
    ...
    def post(self, request):
        ...
        try:
            ...
            messages.success(request, f"Device '{pi_data['pi_name']}' added successfully with {len(pi_data['meters'])} meters!")
...
class EditPiView(View):
    """Edit existing Pi configuration - matches edit_device.html template"""
    
    def get(self, request, pi_id):
        """Display edit Pi form"""
        pi_config = self._get_pi_config(pi_id)
        if not pi_config:
            messages.error(request, "Device configuration not found")
            return redirect('device_config:device_config')
        
        context = {
            'device': pi_config,  # Use 'device' to match DCMS template
            'pi_config': pi_config,  # Keep for compatibility
            'pi_id': pi_id,
    ...
    def post(self, request, pi_id):
        ...
        try:
            ...
            messages.success(request, f"Device '{configs[pi_id]['pi_name']}' updated successfully!")
            return redirect('device_config:device_config')
    ...
    def post(self, request, pi_id):
        ...
        try:
            ...
            if pi_id < len(configs):
                ...
                messages.success(request, f"Device '{pi_name}' deleted successfully!")
            else:
                messages.error(request, "Device configuration not found")
...
class DeployConfigView(View):
    ...
    def post(self, request, pi_id):
        ...
        try:
            pi_config = self._get_pi_config(pi_id)
            if not pi_config:
                return JsonResponse({'success': False, 'error': 'Device configuration not found'})
            
            # Test connection first
            if not self._test_ssh_connection(pi_config):
                return JsonResponse({'success': False, 'error': 'Cannot connect to device via SSH'})
...
def main():
    """Create the complete views file"""

    views_file = Path(
        '/home/isha/deepak/MFM_offline_setup/meter_dashboard/device_config/views.py')

    print("📝 Creating complete device_config views...")

    try:
        with open(views_file, 'w', encoding='utf-8') as f:
            f.write(create_complete_views())

        print("✅ Created complete views.py file")

        print("""
🎉 Device Config views created successfully!

✅ All view classes match the URL patterns
✅ Views compatible with DCMS templates  
